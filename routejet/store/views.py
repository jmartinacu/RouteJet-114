...
from .utils import stripe_payment, reduce_order_num_products_cart
from .utils import reduce_order_num_products_not_cart
...
def search_products(request):
    ...
    return render(
        request,
        'store/product_filter.html',
        {'results': results, 'query': query, 'cart': cart}
    )
    ...
    if form.is_valid():
        ...
        if search_query:
            ...
            orders = orders.filter(
                ...
                # Buscar por opciones legibles
                Q(state__in=[
                    k for k, v in state_mapping.items()
                    if v.lower().startswith(search_query.lower())
                ]) |
...
def get_products_by_orders(orders):
    products = []
    for order in orders:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if order.email != request.user.email:
                form.add_error(
                    'order_id', 'La orden no está relacionada con el usuario actual.')
                return render(request, 'store/create_claim.html', {'form': form})
            ...
            if existing_claim:
                form.add_error(
                    'order_id', 'Ya existe una reclamación para esta orden.')
                return render(request, 'store/create_claim.html', {'form': form})
