...
def task_send_email_order_created(order_id):
    ...
    message = f'Estimado cliente {order.first_name}, \n\n' \
              ...
              f'El ID de tu pedido es {order.id},' \
              f'puedes ver los detalles del pedido con tu email en el historial de pedidos.'
    mail_sent = send_mail(
...
def task_change_state_orders_every_day():
    ...
    for order in orders:
        order_state_pa = order.state == 'PA'
        if (order_state_pa and order.paid) or (order_state_pa and order.payment_on_delivery):
            order.state = 'OTW'
            order.save()
